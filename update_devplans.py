...
if jsonResponse.status != 200:
	...
	for p in jsonObj:
		for k in p:
			p[k.lower()] = p.pop(k)
		planIds.append(int(p['devplan_id']))

	client = MongoClient(os.getenv("OPENSHIFT_MONGODB_DB_URL"))
	db = client.raleigh
	collection = db.devplans
	print str(db.devplans.count())
	currentCnt = db.devplans.count()
	collection.remove({"properties.devplan_id": {"$in": planIds}})
	print str(db.devplans.count())
	removeCnt = currentCnt - db.devplans.count()
